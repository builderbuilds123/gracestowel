...
**And** it should parse the `order_id` correctly
...
- Stripe `increment_authorization` can only handle amounts the issuer allows. Fallback strategy (Story 7.1 in future) allows re-auth if needed, but for now, fail if increment is denied.

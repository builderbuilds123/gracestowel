...
**I want** the payment and order creation process to rely solely on server-side pricing logic,
**So that** malicious actors cannot manipulate order amounts or contents.

**Acceptance Criteria:**

**Given** a request to create a payment intent
**When** the system calculates the amount
**Then** it should use the total from the canonical Medusa Cart
**And** it should ignore any client-provided price or amount fields
**And** the order creation workflow should validate line item prices against the Medusa Cart
**And** any discrepancy between client and server amounts should result in a failure

...
export default async function fixProductPrices({ container }: ExecArgs) {
  ...
  const regionMap = new Map(regions.map(r => [r.currency_code.toLowerCase(), r.id]));
  ...
  for (const product of products) {
    ...
    for (const variant of product.variants || []) {
      ...
      const existingPrices = (variant as any).prices || [];
        ...
        const priceSet = await pricingModuleService.createPriceSets([
          {
            prices: [
              {
                amount: priceConfig.usd * 100, // Convert to cents
                currency_code: "usd",
              },
              {
                amount: priceConfig.eur * 100,
                currency_code: "eur",
              },
              {
                amount: priceConfig.cad * 100,
                currency_code: "cad",
              },
            ],
          },

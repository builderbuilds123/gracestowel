...
interface PromoCodeInputProps {
  ...
  appliedCodes: Array<{ code: string; discount: number }>;
...
/**
 * Badge displaying an applied promo code with remove button
 */
  ...
  return (
    <div className="flex items-center justify-between bg-green-50 border border-green-200 rounded-md px-3 py-2" data-testid={`applied-promo-${code}`}>
      <div className="flex items-center gap-2">
        ...
        {discount > 0 && (
          <span className="text-sm text-green-600">-{formattedDiscount}</span>
        )}
      </div>
      <button
        type="button"
